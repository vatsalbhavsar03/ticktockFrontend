...
const Checkout = () => {
    ...
    const placeOrderAndPayment = async (transactionId = "", paymentStatus = "Pending") => {
        const userId = localStorage.getItem("UserId");
        try {
            // Create Order
            const orderRes = await axios.post(`https://localhost:7026/api/Order/CreateOrder`, {
                order: {
                    userId,
                    phone: user.mobile,
                    address,
            ...
            // Clear cart
            for (const item of items) {
                await axios.delete(`${API_BASE_URL}/Cart/${item.cartitemId}/DeleteCart`);
            }
    ...
    const handlePlaceOrder = async () => {
        ...
        if (paymentMethod === "cod") {
            await placeOrderAndPayment("N/A", "COD");
        } else {
            const amountInPaise = total * 100;
